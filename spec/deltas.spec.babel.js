...
import Deltas from '../src/deltas.babel.js';

const staggerStep = (base, step) => {
  const result = (index, total) => {
...
describe('`deltas` ->', function () {
  ...
  describe('`_parseProperties` / deltas parsing ->', function() {
    ...
    it('should parse stagger options on delta`s tweens', function () {
      ...
      var options = {
        el: {},
        x: { 20: 40 },
        z: { 10: 30, duration: staggerStep(500, 1500) },
        t: { path: 'M100,200, L300,250' },
        f: 5,
        y: { '200': 300, delay: 200, path: 'M0,0 L200,200' },
        index: index
  ...
  describe('`_upd_deltas` function ->', function() {
    ...
    it('should call the pipe function', function () {
      var pipeObj = {};
      var customProperties = {
        pipe: function() {},
        pipeObj: pipeObj
      ...
      expect(deltas._pipe.calls.mostRecent().args[0]).toBe(deltas._el);
      expect(deltas._pipe.calls.mostRecent().args[1].props).toBe(deltas._supportProps);
      expect(deltas._pipe.calls.mostRecent().args[1].pipeObj).toBe(deltas._pipeObj);
      expect(deltas._pipe.calls.mostRecent().args[2]).toBe(progress);
      expect(deltas._pipe.calls.mostRecent().args[3]).toBe(progress);
      expect(deltas._pipe.calls.mostRecent().args[4]).toBe(isForward);
    });
